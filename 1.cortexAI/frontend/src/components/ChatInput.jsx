...
function ChatInput() {
  ...
  const handleSendMessage = async () => {
    ...
    if (conversation && conversation.title === "New Chat") {
      updateConversation({ id: conversation?._id, title: promptText.slice(0, 30) }).catch(() => {})
      dispatch(setConvTitle({ conversationId: conversation?._id, title: promptText.slice(0, 30) }))
    }

    const formData = new FormData()
    formData.append("prompt", promptText)
    formData.append("conversationId", conversation?._id || "default-conv")
    ...
    try {
      ...
      const aiContent = data?.answer || "Hello! I am CortexAI. How can I assist you today?"
      dispatch(setArtifacts(data?.artifacts || []))
      dispatch(addMessage({ role: "assistant", content: aiContent, images: data?.images || [] }))
    } catch (err) {
      console.error("handleSendMessage error:", err)
      dispatch(setIsLoading(false))
      dispatch(addMessage({ role: "assistant", content: "Hello! I am CortexAI. Ready to assist you." }))
    }
  }
