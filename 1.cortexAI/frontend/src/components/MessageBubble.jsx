  ...
  leading-relaxed
        ...
        <Markdown
          ...
          components={{
            ...
            a: ({ href, children }) => {
              const serverUrl = import.meta.env.VITE_SERVER_URL || "http://localhost:8000"
              const isApiFile = href?.includes("/api/agent/files/") || href?.includes("/files/")
              let finalHref = href
              if (href && href.startsWith("/")) {
                finalHref = `${serverUrl}${href}`
              }

              return (
                <a
                  href={finalHref}
                  download={isApiFile ? true : undefined}
                  target="_blank"
                  rel="noreferrer"
                  className="text-indigo-400 underline inline-flex items-center gap-1 font-medium hover:text-indigo-300 transition-colors"
                >
                  {children}
